package com.ftninformatika.bisis.rest_service.repository.mongo.implementations;

import com.ftninformatika.bisis.circ.Lending;
import com.ftninformatika.bisis.circ.LendingDateOption;
import com.ftninformatika.bisis.rest_service.repository.mongo.interfaces.LendingRepositoryCustom;
...
public class LendingRepositoryImpl implements LendingRepositoryCustom {
    ...
    public Integer getLendMemberCountDistinctByDate(Date start, Date end, String location) {
        ...
        retVal = distinctCountByDay(lendingList, LendingDateOption.LEND_DATE);
    ...
    public Integer getReturnMemberCountDistinctByDate(Date start, Date end, String location) {
        ...
        List<Lending> lendingList = mongoTemplate.find(q, Lending.class);
        retVal = distinctCountByDay(lendingList, LendingDateOption.RETURN_DATE);

        return retVal;
    }

    public static Integer distinctCountByDay(List<Lending> lendingList, LendingDateOption whichDate) {
        Integer retVal = 0;
        Map<Date, List<String>> dateUserIdMap = new HashMap<>();
        for (Lending l : lendingList) {
            Date d = null;
            if (whichDate.equals(LendingDateOption.LEND_DATE))
                d = DateUtils.getStartOfDay(l.getLendDate());
            else if (whichDate.equals(LendingDateOption.RESUME_DATE))
                d = DateUtils.getStartOfDay(l.getResumeDate());
            else if (whichDate.equals(LendingDateOption.RETURN_DATE))
