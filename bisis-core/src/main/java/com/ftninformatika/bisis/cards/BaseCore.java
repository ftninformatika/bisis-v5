...
public class BaseCore {
    ...
    public BaseCore(int docID, Record rec, String code) {
        this.code = code;
        this.docID = docID;
        this.rec = rec;
    }

    public BaseCore(Record rec) {
        this.rec = rec;
    }
    ...
    public  Map<String,Object> struktura(){
        ...
        root.put("nemaSignature", Messages.getString("CARDS_SIGNATURE_MISSING"));
        root.put("odrednica", Messages.getString("CARDS_HEADING_NOT_DEFINED"));
        root.put("promocija", Messages.getString("CARDS_PROMOTION"));
        root.put("mentor", Messages.getString("CARDS_MENTOR"));
        root.put("komisija", Messages.getString("CARDS_COMMISSION"));
        root.put("odbrana", Messages.getString("CARDS_CONCEPT_DEFEND"));
        root.put("sadrzaj", Messages.getString("CARDS_CONCEPT_CONTENT"));
        root.put("mrtitle", Messages.getString("CARDS_MAIN_TITLE"));
        root.put("crtitle", Messages.getString("CARDS_CONCEPT_TITPE"));
        root.put("ortitle", Messages.getString("CARDS_OM_REAL_TITLE"));
        root.put("rtitleshead", Messages.getString("CARDS_REAL_TITLE_ON_SECONDARY"));
        root.put("rtitleabovetext", Messages.getString("CARDS_MAIN_TITLE_ON_TEXT"));
        root.put("currtitle", Messages.getString("CARDS_MAIN_CURRENT_TITLE"));
        root.put("hrbtitle", Messages.getString("CARDS_HRB_TITLE"));
        root.put("ssltitle", Messages.getString("CARDS_OTHER_SECONDARY_MAIN_TITLE"));
        root.put("addtitle", Messages.getString("CARDS_ADDED_TITLE"));
        root.put("f423mustHave200", Messages.getString("CARDS_FIELD_200_OR_500_MANDATORY_IN_FIELD_423"));
        root.put("f423content", Messages.getString("CARDS_FIELDS_200_OR_500_AND_7_OR_503_IN_423"));
        root.put("f423wrongFieldOrder", Messages.getString("CARDS_WRONG_ORDER_SEC_FIELDS_423"));
        ...
        for (int i=0; i < in.length(); i++)  {
            ...
            if (pocetak){
                ...
                if (i<(in.length()-4) && 	in.substring(i,i+5).equals("<sig>")){
                    ...
                    int pom=n-i+bk;
                    if (pom-1> ReportCore.bkmax && !sig ) {  // rec je duza od duzine reda
                        out +="<BR>\n";
                        bk = 1;
    ...
    public String formatStrana(String in){
        ...
        while(i<in.length()){
            ...
            if (np){
                np=false;
                for(int k=br;k<ReportCore.brmax;k++ ){
            ...
            else if (br==ReportCore.brmax){

                String in1 = StringUtils.replace(in.substring(i), "&nbsp;", " ");

                if (in1.length() > ReportCore.bkmax || (in1.indexOf("<BR>")!=-1)) {
                    if (bold && !red.startsWith("<B>") ){
                        stranica+="</B>";
                    }

                    strana++;bk=1;
                    if (ReportCore.nextPage!=""){
                        out+=stranica+ReportCore.izlaz1+"<BR>\n"+rightAlign(String.valueOf(strana))+"<BR>\n";
    ...
    private String rightAlign(String in) {
        ...
        if (!in.equals("")) {
            ...
            for (int j=bk; j < (ReportCore.bkmax-in.trim().length()); j++)
                blanko += "&nbsp;";
            in = blanko + in;
            //     in = blanko + in + "<BR>\n";
        }
